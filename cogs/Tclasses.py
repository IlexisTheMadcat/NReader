...
class ImagePageReader:
    ...
    async def setup(self):
        ...
        while True:
            try:
                def check(i):
                    return i.message.id==conf.id and i.user.id==self.ctx.author.id

                interaction = await self.bot.wait_for("button_click", timeout=60, check=check, bypass_cooldown=True)
    ...
    async def start(self):
        ...
        while True:
            try:
                def check(i):
                    return i.message.id==self.active_message.id and i.user.id==self.ctx.author.id
