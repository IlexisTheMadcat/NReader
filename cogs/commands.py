...
class Commands(Cog):
    ...
    async def doujin_info(self, ctx, code="random"):
        ...
        print(f"[] {ctx.author} ({ctx.author.id}) looked up `{code}`"
              f"{' containing underage characters in an Unrestrictedserver.' if ('lolicon' in doujin.tags or 'shotacon' in doujin.tags) and ctx.guild and lolicon_allowed else '.'}")
    ...
    async def search_doujins(self, ctx, *, query: str = ""):
        ...
        results: dict = nhentai_api.search(query=query+f"{' -lolicon -shotacon' if ctx.guild and not lolicon_allowed else ''} {appendage}", sort='popular', page=page)
        ...
        emb.set_footer(
            text=f"Loading more details... [{' '*len(results.doujins)}]")
            ...
            if ind%5 == 0 and ind!=0:
                ...
                for ind2, dj2 in enumerate(results.doujins):
                    message_part2.append(
                        ...
                        f"{language_to_flag(dj2.languages) if isinstance(dj2, Doujin) else '🏳❔'} | "
                ...
                emb2.set_author(
                    ...
                    icon_url="https://cdn.discordapp.com/emojis/810936543401213953.gif?v=1")
                emb2.set_footer(
                    text=f"Loading more details... [{'|'*ind}{' '*(len(results.doujins)-ind)}]")
    ...
    async def whitelist(self, ctx, mode=None):
        ...
        if "UnrestrictedServers" not in self.bot.user_data["UserData"][str(ctx.author.id)]["Settings"]:
            self.bot.user_data["UserData"][str(ctx.author.id)]["Settings"]["UnrestrictedServers"] = []
        ...
        if not mode:
            if self.bot.user_data["UserData"][str(ctx.author.id)]["Settings"]["UnrestrictedServers"]:
                message_part = []
                for i in self.bot.user_data["UserData"][str(ctx.author.id)]["Settings"]["UnrestrictedServers"]:
